...
internal class SqlTokens
{
    ...
    private const string Keywords =
        "ADD,ALTER,AS,ASC,AUTHORIZATION,BACKUP,BEGIN,BREAK,BROWSE,BULK,BY,CASCADE,CHECK,CHECKPOINT,CLOSE,CLUSTERED,COLLATE," +
        "COLUMN,COMMIT,COMPUTE,CONSTRAINT,CONTAINS,CONTAINSTABLE," + "CONTINUE,CREATE,CURRENT,CURSOR,DATABASE," +
        ...
        "INTERSECT,INTO,IS,ISABOUT,KEY,KILL,LINENO,LOAD," + "NATIONAL,NOCHECK,NONCLUSTERED,OF,OFF," +
        "OFFSETS,ON,OPEN,OPENDATASOURCE,OPENQUERY,OPENROWSET,OPENXML," +
        "OPTION,ORDER,OVER,PERCENT,PLAN,PRECISION," + "PRIMARY,PRINT,PROC,PROCEDURE,PUBLIC,RAISERROR,READ," +
        "READTEXT,RECONFIGURE,REFERENCES,REPLICATION,RESTORE," +
        "RESTRICT,RETURN,REVOKE,ROLLBACK,ROWCOUNT,ROWGUIDCOL," + "RULE,SAVE,SCHEMA,SELECT,SET,SETUSER,SHUTDOWN," +
        "STATISTICS,TABLE,TEXTSIZE,THEN,TO,TOP,TRAN,TRANSACTION," +
        "TRIGGER,TRUNCATE,TSEQUAL,UNION,UNIQUE,UPDATE,UPDATETEXT," +
        "USE,VALUES,VARYING,VIEW,WAITFOR,WEIGHT,WHEN,WHERE,WHILE," + "WITH,WRITETEXT,CURRENT_DATE,CURRENT_TIME" +
        ...
        ",ISOLATION,LEVEL,LOCAL,DATE,MINUTE,TRANSLATION" + ",TRUE,NO,ONLY,WORK,OUTPUT" +
    ...
    private void AddTokens(string tokens, YukonLexer.TokenKind tokenkind)
    {
        ...
        foreach (var t in tokens)
        {
            if (t == ',')
            {
                ...
                this.m_Words.TryAdd(s, tokenkind);
