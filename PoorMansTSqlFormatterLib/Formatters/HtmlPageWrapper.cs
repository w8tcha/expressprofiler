...
namespace PoorMansTSqlFormatterLib.Formatters
{
    ...
    {
        private readonly ISqlTreeFormatter _underlyingFormatter;

        public HtmlPageWrapper(ISqlTreeFormatter underlyingFormatter)
        {
            ArgumentNullException.ThrowIfNull(underlyingFormatter);
        ...
        public string ErrorOutputPrefix {
            get => _underlyingFormatter.ErrorOutputPrefix;
            set => throw new NotSupportedException("Error output prefix should be set on the underlying formatter - it cannot be set on the Html Page Wrapper.");
        }

        public string FormatSQLTree(PoorMansTSqlFormatterLib.ParseStructure.Node sqlTree)
        {
            var formattedResult = _underlyingFormatter.FormatSQLTree(sqlTree);
