...
namespace PoorMansTSqlFormatterLib
{
    internal class ParseTree : NodeImpl
    {
        ...
        {
            var newElement = NodeFactory.CreateNode(newElementName, newElementValue);
        ...
        {
            var newElement = SaveNewElement(newElementName, newElementValue);
            SetError();
            return newElement;
        }
        ...
        internal void StartNewContainer(string newElementName, string containerOpenValue, string containerType)
        {
            ...
            var containerOpen = SaveNewElement(SqlStructureConstants.ENAME_CONTAINER_OPEN, "");
        ...
        internal void StartNewStatement(Node targetNode)
        {
            ...
            var newStatement = SaveNewElement(SqlStructureConstants.ENAME_SQL_STATEMENT, "", targetNode);
        ...
        private void MigrateApplicableCommentsFromContainer(Node previousContainerElement)
        {
            ...
            var migrationCandidate = previousContainerElement.Children.Last();

            //keep track of where we're going to be prepending - this will change as we go moving stuff.
            var insertBeforeNode = CurrentContainer;
            ...
            while (migrationCandidate != null)
            {
                    ...
                    )
                {
                        ...
                        )
                    {
                        ...
                        {
                            var movingNode = migrationContext.Children.Last();
        ...
        internal void ConsiderStartingNewStatement()
        {
            ...
            var previousContainerElement = CurrentContainer;

            //context might change AND suitable ancestor selected
            var nextStatementContainer = EscapeAndLocateNextStatementContainer(false);
            ...
            {
                var inBetweenContainerElement = CurrentContainer;
        ...
        {
            var nextStatementContainer = EscapeAndLocateNextStatementContainer(true);
        ...
        {
            foreach (var testElement in containerNode.Children)
        ...
        {
            var candidateContainer = CurrentContainer;
            while (levelsUp > 0)
            {
